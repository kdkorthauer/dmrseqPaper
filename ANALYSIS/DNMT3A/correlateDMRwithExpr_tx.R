...
corrWithExpression <- function(dmrs, time, time2=NULL, tiss1, tiss2, METHOD, 
 ...
 #  dmrs <- dmrs[abs(dmrs$beta)> 0.10,] 
   ...
   expr <- read.table("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/DATA/RNAseq/GSE61969_cufflinks.gene.counts.txt",
 ...
 gene.annot.filename <- "/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/gene.annot.rda"
 
  # table of UCSC tx ids to Ensembl (obtained from UCSC table browser)
  knownToEnsembl_mm10 <- read.table("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/knownToEnsembl_mm10.txt",
 	stringsAsFactors=FALSE)
 colnames(knownToEnsembl_mm10) <- c("tx_id", "ensembl")
 knownToEnsembl_mm10$ensembl_id <- unlist(sapply(strsplit(knownToEnsembl_mm10$ensembl, "\\."), function(y) y[[1]]))
  
   
 if(!file.exists(gene.annot.filename)){
   # get GRanges object with positions of all genes with Ensembl IDs
   # this uses the mouse genome since it is intended for use on the ENCODE dataset
   ...
   Ensembl_tx_to_gn <- function(geneNames, organism="mmusculus_gene_ensembl"){
		  require(biomaRt);
   ...
   if(!file.exists(gene.annot.filename)){
	   ...
	   save(genes, file=gene.annot.filename)
   }else{
   ...
   x <- match(mcols(genes)$Gene, exprIDs)
   x <- x[!is.na(x)]   
   x <- unique(x)
   ...
   y <- match(exprIDs, mcols(genes)$Gene)
   genes <- genes[y,]	
...
if (length(c1)>0 & length(c2)>0){
   ...
   annot <- unique(annot)
   ucsc.ens <- match(annot$tx_id, knownToEnsembl_mm10$tx_id)
   annot$ensembl_id <- knownToEnsembl_mm10$ensembl_id[ucsc.ens]
 ...
 for(rnk in topRanks){
  if (fdr & ct > 1 & 2 < 1){
   ...
   #which.trans <- match(annot[overlaps2@to,]$ensembl_id, genes$name)
...
#   }
  ...
  if(length(overlaps2) > 0){  
   ...
   #meth.diff.t <- topDMRs$stat[overlaps2@from[!xt]]
...
save(rank.df, file=paste0(result.file.prefix, "/", subfolder, "/oddsByRanking.", METHOD, ".n", 
   ...
   if (grepl("DSS", METHOD) | grepl("BSmooth", METHOD) | grepl("metilene", METHOD)){
   		pfile1 <- list.files(paste0(path="/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/dmrseq_pkg/", subfolder, "/"),
   						pattern=paste0("oddsByRanking.dmrseq.n", 
		   					sampleSize, ".", cond, ".", num.dmrs, "DMRS.top")) 
   		pfile2 <- list.files(paste0(path="/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/dmrseq_pkg/", subfolder, "/"),
   						pattern=paste0("_min", min.length, "_max", max.length, ".RData")) 		  
   
		if (length(pfile1[pfile1 %in% pfile2]) == 1){	   					 					
		 pfile <- paste0(paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/dmrseq_pkg/", subfolder, "/"), 
   ...
   if(fdr){
       bfile1 <- list.files(path=paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/BSmooth_default/", "odds_statistic2_default", "/"),
							pattern=paste0("oddsByRanking.BSmooth_default.n", 
								sampleSize, ".", cond, ".", num.dmrs, "DMRS.top")) 
	   bfile2 <- list.files(path=paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/BSmooth_default/", "odds_statistic2_default", "/"),
							pattern=paste0("_min", min.length, "_max", max.length, ".RData")) 		  
	   if (length(bfile1[bfile1 %in% bfile2]) == 1){	   					 					
	   bfile <- paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/BSmooth_default/", "odds_statistic2_default", "/", 
				  bfile1[bfile1 %in% bfile2])
	   }else{
		  bfile <- "null"
	   }

	   dfile1 <- list.files(path=paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/DSS_default/", "odds_statistic2_default", "/"),
							pattern=paste0("oddsByRanking.DSS_default.n", 
								sampleSize, ".", cond, ".", num.dmrs, "DMRS.top")) 
	   dfile2 <- list.files(path=paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/DSS_default/", "odds_statistic2_default", "/"),
							pattern=paste0("_min", min.length, "_max", max.length, ".RData")) 		  
	   if (length(dfile1[dfile1 %in% dfile2]) == 1){	   					 					
	   dfile <- paste0("/n/irizarryfs01_backed_up/kkorthauer/WGBS/DNMT3A/RESULTS/DSS_default/", "odds_statistic2_default", "/", 
    ...
    if(!fdr){ 
		gp <- ggplot(aes(x=ranks, y=log2(odds)), 
   		...
   		if ("propDE" %in% colnames(ranks.all)){
   			...
   			DE.cats <- melt(ranks.all[,c(11:14, 17)])
