...
if (num.dmrs > 0){
    # don't re-simulate data; use what was simulated for the Perm method
    # load(paste0(result.file.prefix0, "/sim.data.n", sampleSize, 
	#				   ".", cond, ".", chromosome, ".rda")) 
...
if (METHOD=="Metilene" | METHOD=="metilene"){
	...
	if (!file.exists(paste0(result.file.prefix, met.file.out))| 
		file.size(paste0(result.file.prefix, met.file.out)) == 0){
  		system(paste0(metilene.path,
  			" -a g1 -b g2 -t ", workers, " ", "-m ", minNumRegion, " -d 0 -M ",
  			 maxGap, " ", met.file, " > ", result.file.prefix, met.file.out))
  	}
...
if (METHOD=="BSmooth" | METHOD=="bsmooth"){
   ...
   #result.file.prefix <- paste0(result.file.prefix0, "/bsmooth_binSim")
  ...
  ##### Code to run BSmooth
   ...
   if (num.dmrs == 0){
	   smooth.file.name=paste0(result.file.prefix, "/smooth.BSmooth.n", 
		   sampleSize, ".", cond, ".RData")
	   dmr.file.name=paste0(result.file.prefix, "/dmrs.BSmooth.n", 
	   		sampleSize, ".", cond, ".Rdata")

   }else{
	   smooth.file.name=paste0(result.file.prefix, "/smooth.BSmooth.n", 
		   sampleSize, ".", cond, ".", num.dmrs, "DMRS.RData")
	   dmr.file.name=paste0(result.file.prefix, "/dmrs.BSmooth.n", 
	   	   sampleSize, ".", cond, ".", num.dmrs, "DMRS.Rdata")
   }
